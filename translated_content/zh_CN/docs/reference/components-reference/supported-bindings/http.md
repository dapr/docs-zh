...
[服务调用 API]({{% ref service_invocation_api.md %}}) 允许调用非 Dapr HTTP 端点，是推荐的方式。阅读 ["如何操作：使用 HTTP 调用非 Dapr 端点"]({{% ref howto-invoke-non-dapr-endpoints.md %}}) 了解更多信息。
...
| `maxResponseBodySize`| N | 输出 | 要读取的响应的最大长度。整数被解释为字节；为方便起见，可以添加 `Ki, Mi, Gi`（SI）或 `k | M | G`（十进制）等单位。默认值为 `100Mi` | "1Gi"、"100Mi"、"20Ki"、"200"（字节） |
...
元数据字段 **MTLSRootCA**、**MTLSClientCert** 和 **MTLSClientKey** 用于配置 mTLS 认证。
要使用 mTLS 认证，必须提供所有三个字段。有关更多详细信息，请参阅 [mTLS]({{% ref "#using-mtls-or-enabling-client-tls-authentication-along-with-https" %}})。您也可以仅提供 **MTLSRootCA**，以启用由自定义 CA 签名的证书的 **HTTPS** 连接。有关更多详细信息，请参阅 [HTTPS]({{% ref "#install-the-ssl-certificate-in-the-sidecar" %}}) 部分。
{{% /alert %}}


## Binding 支持

此组件支持 **输出 binding**，支持以下 [HTTP 方法/动词](https://www.w3.org/Protocols/rfc2616/rfc2616-sec9.html)：
...
| 字段              | 必填 | 详情 | 示例 |
|--------------------|:--------:|---------|---------|
| `statusCode`         | Y        | [HTTP 状态码](https://www.w3.org/Protocols/rfc2616/rfc2616-sec10.html) | `200`、`404`、`503` |
...
{{% alert title="注意" color="primary" %}}
任何以大写字母开头的元数据字段都将作为请求头部传递。
例如，默认的内容类型为 `application/json; charset=utf-8`。可以通过设置 `Content-Type` 元数据字段来覆盖。
{{% /alert %}}
...
1. 如果需要添加自定义 TLS 证书，请参阅 [如何操作：在 Dapr 边车中安装证书]({{% ref install-certificates %}})，在边车中安装 TLS 证书。
...
边车可以从各种源读取 TLS 证书。有关更多信息，请参阅 [如何操作：将 Pod 卷挂载到 Dapr 边车]({{% ref kubernetes-volume-mounts %}})。在此示例中，我们将 TLS 证书存储为 Kubernetes secret。
...
{{% alert title="注意" color="primary" %}}
还可以使用 **MTLSRootCA** 元数据选项配置 HTTPS binding 支持。这将把指定的证书添加到 binding 的受信任证书列表中。两种方法没有特定的偏好。虽然 **MTLSRootCA** 选项易于使用且不需要对边车进行任何更改，但它仅接受一个证书。如果您需要信任多个证书，则需要[按照上述步骤在边车中安装它们]({{% ref "#install-the-ssl-certificate-in-the-sidecar" %}})。
{{% /alert %}}
...
- [如何操作：在 Dapr 边车中安装证书]({{% ref install-certificates %}})
