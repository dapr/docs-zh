...
一个构建块可以使用任意组件组合。例如，[Actor]({{% ref "actors-overview" %}})和[状态管理]({{% ref "state-management-overview" %}})构建块都使用[状态组件](https://github.com/dapr/components-contrib/tree/master/state)。
...
对于任何向应用返回数据的组件，建议相应地设置 Dapr 边车（进程或容器）的内存容量，以避免潜在的 OOM 恐慌。例如，在 Docker 中使用 `--memory` 选项。对于 Kubernetes，使用 `dapr.io/sidecar-memory-limit` 注解。对于进程，这取决于操作系统和/或进程编排工具。*
{{% /alert %}}
...
这些 YAML 文件遵循通用的 [Dapr 组件架构]({{% ref "component-schema" %}})，但每个都特定于组件规范。

重要的是要理解，组件规范值，特别是规范 `metadata`，可能会在同一组件类型的组件之间发生变化，例如在不同的状态存储之间，并且一些设计时规范值可以在运行时向组件 API 发出请求时被覆盖。因此，强烈建议查看[组件规范]({{% ref "components-reference" %}})，特别注意用于设置与组件交互的 metadata 的请求示例负载。
...
但是，可插拔组件非常适合您想要创建自己的未包含在 Dapr 项目中的私有组件的场景。
例如：
...
启用[热重载功能]({{% ref "support-preview-features" %}})后，组件可以在运行时进行"热重载"。
...
名称解析组件与[服务调用]({{% ref "service-invocation-overview" %}})构建块一起使用，与托管环境集成并提供服务到服务的发现。例如，Kubernetes 名称解析组件与 Kubernetes DNS 服务集成，自托管模式使用 mDNS，VM 集群可以使用 Consul 名称解析组件。
...
发布订阅代理组件是消息代理，可以作为[发布和订阅]({{% ref pubsub-overview %}})构建块的一部分在服务之间传递消息。
...
状态存储组件是数据存储（数据库、文件、内存），作为[状态管理]({{% ref "state-management-overview" %}})构建块的一部分存储键值对。
...
外部资源可以连接到 Dapr，以便触发应用程序上的方法或从应用程序调用，作为[绑定]({{% ref bindings-overview %}})构建块的一部分。
...
[密钥]({{% ref "secrets-overview" %}})是您想要防范未经授权访问的任何私人信息。密钥存储用于存储可以在应用程序中检索和使用的密钥。
...
[加密]({{% ref cryptography-overview %}})组件用于执行加密操作，包括加密和解密消息，而不会向应用程序暴露密钥。
...
Dapr 为开发人员提供了一种抽象与大语言模型 (LLM) 交互的方法，具有内置的安全性和可靠性功能。使用[对话]({{% ref conversation-overview %}})组件向不同的 LLM 发送提示以及对话上下文。
...
Dapr 允许将自定义[中间件]({{% ref "middleware" %}})插入到 HTTP 请求处理管道中。中间件可以在请求路由到用户代码之前或响应返回给客户端之前对 HTTP 请求执行其他操作（例如身份验证、加密和消息转换）。中间件组件与[服务调用]({{% ref "service-invocation-overview" %}})构建块一起使用。
