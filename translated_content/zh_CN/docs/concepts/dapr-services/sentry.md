...
Dapr Sentry 服务管理服务间的 mTLS 并充当证书颁发机构。它生成 mTLS 证书并将其分发给任何正在运行的边车。这使边车能够使用加密的 mTLS 流量进行通信。更多信息请阅读[边车到边车通信概述]({{% ref "security-concept#sidecar-to-sidecar-communication" %}})。

## 自托管模式

Sentry 服务 Docker 容器不会作为 [`dapr init`]({{% ref self-hosted-with-docker %}}) 的一部分自动启动。但可以按照设置[双向 TLS]({{% ref "mtls#self-hosted" %}})的说明手动执行它。

如果您以 [slim-init 模式]({{% ref self-hosted-no-docker %}}) 运行，它也可以作为进程手动运行。
